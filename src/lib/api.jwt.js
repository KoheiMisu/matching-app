...
export default class JWT {
  ...
  getToken = credentials => new Promise(async (resolve, reject) => {
    ...
    if (credentials && typeof credentials === 'object' && credentials.fb_name && credentials.fb_id) {
      this.apiCredentials.fb_name = credentials.fb_name;
      this.apiCredentials.fb_id = credentials.fb_id;
    ...
    if (!this.apiCredentials || !this.apiCredentials.fb_name || !this.apiCredentials.fb_id) {
      return reject({
        data: { status: 403 },
    ...
    return AppAPI[APIConfig.tokenKey].post(null, {
      fb_name: this.apiCredentials.fb_name,
      fb_id: this.apiCredentials.fb_id,
    }).then(async (res) => {
      if (!res.token) {
        return reject(res);
      }
